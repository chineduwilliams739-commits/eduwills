...
async function callGroq(key, prompt) {
  ...
  if (!r.ok) {
    ...
    const e = new Error(`GROQ_${r.status}`); e.status = r.status; e.detail = text.slice(0, 500); throw e;
  }
...
async function callOpenRouter(key, prompt) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'HTTP-Referer': 'https://chineduwilliams739-commits.github.io/eduwills/',
      'X-Title': 'EduWills'
    },
    body: JSON.stringify({
  ...
  if (!r.ok) {
    const text = await r.text();
    const e = new Error(`OPENROUTER_${r.status}`); e.status = r.status; e.detail = text.slice(0, 500); throw e;
  }
  const d = await r.json();
  return d?.choices?.[0]?.message?.content || '';
}
    ...
    try {
      ...
      for (const [provider, call] of providers) {
        try {
          ...
          if (text) return json(res, 200, { provider, text });
        } catch (e) {
          failures.push({ provider, status: e.status || 0, code: e.message || 'provider_error' });
        }
      }
      return json(res, 503, { error: 'AI providers unavailable', failures });
    } catch (e) {
      return json(res, 500, { error: 'AI router failure' });
    }
  }
);
