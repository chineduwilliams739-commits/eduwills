  ...
  async function login(e: React.FormEvent) {
    ...
    try {
      ...
      if (!admin.exists() && credential.user.uid === OWNER_ADMIN_UID) {
        await setDoc(adminRef, { uid: credential.user.uid, email: credential.user.email || email.trim(), displayName: credential.user.displayName || '', createdAt: serverTimestamp(), role: 'owner' }, { merge: true });
      } else if (!admin.exists()) {
        await auth.signOut(); setError('This Firebase account is not authorized as an EDUWILLS administrator.'); setLoading(false); return;
      }
