...
export default function AdminPage() {
  ...
  useEffect(() => onAuthStateChanged(auth, async user => {
    ...
    try {
      ...
      await load();
    } catch (e: any) { setError(e?.code === 'permission-denied' ? 'Firebase denied Admin access. The Admin UID is not authorized by the current rules.' : 'Could not load Admin data.'); setLoading(false); }
  }), []);
  ...
  const exportUsers = () => {
    const rows = [['Name', 'Username', 'Phone', 'Categories', 'Category expiry policy', 'Activated', 'Books', 'WilliToken expiry']];
    users.forEach(u => { const latest = userTokens(u.uid || u.id)[0]; rows.push([u.fullName || '', u.username ? `@${u.username}` : '', u.phone || '', categoriesFor(u).join(' | ') || 'Not set', categoryExpiryText(u), u.activated ? 'Yes' : 'No', String(userBooks(u.uid || u.id).length), formatExpiry(tokenExpiry(latest))]); });
