...
export default {async fetch(request,env){if(request.method==='OPTIONS')return new Response(null,{status:204,headers:CORS});if(request.method!=='POST')return out({error:'POST_REQUIRED'},405);try{await verify(request);const data=await request.json();const prompt=String(data?.prompt||'').trim();const mode=String(data?.mode||'quiz');if(!prompt||prompt.length>120000)return out({error:'INVALID_PROMPT'},400);const failures=[];const system=mode==='chat'?CHAT_SYSTEM:QUIZ_SYSTEM;const max=mode==='chat'?1200:9000;const temperature=mode==='chat'?0.3:0.2;
 ...
 try{const text=await callProvider('https://openrouter.ai/api/v1/chat/completions',env.OPENROUTER_API_KEY,{...promptBody(system,prompt,max,temperature),model:'openrouter/free'},{Authorization:`Bearer ${env.OPENROUTER_API_KEY}`,'Content-Type':'application/json','HTTP-Referer':ORIGIN,'X-Title':'EduWills'});if(text)return out({provider:'openrouter',text})}catch(e){failures.push({provider:'openrouter',error:String(e?.message||e).slice(0,100)})}
 return out({error:'AI_TEMPORARILY_UNAVAILABLE',message:'AI is temporarily unavailable. Please try again shortly.',failures},503);
