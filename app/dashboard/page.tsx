'use client';
import { BookOpen, Clock3, LogOut, Menu, UserRound, WalletCards, X, Sparkles, ArrowRight, LockKeyhole, Bot, CheckCircle2 } from 'lucide-react';
...
export default function DashboardPage(){
 const [mobileOpen,setMobileOpen]=useState(false),[name,setName]=useState('Learner'),[activated,setActivated]=useState(false),[loading,setLoading]=useState(true),[expiry,setExpiry]=useState('');
 ...
 const go=(href:string,n:string)=>{if(locked(n)){setMobileOpen(false);alert('Activate your EDUWILLS account with a WilliToken to unlock this section.');return;}setMobileOpen(false);window.location.assign(href);};
