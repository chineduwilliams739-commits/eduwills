...
export default function ActivationPage() {
  ...
  useEffect(() => {
    ...
    const user = users.find((u: { username: string }) => u.username === current);
    if (user) {
  ...
  const openWhatsApp = () => {
    const text = `Hello EDUWILLS Admin. I want to activate my account. My username is: ${username}. I have made my activation payment and will send my payment receipt/proof here. I will also send the amount paid, date/time and EDUWILLS phone number.`;
    window.open(`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(text)}`, '_blank', 'noopener,noreferrer');
  };

  const activateToken = () => {
    const clean = token.trim().toUpperCase();
    const tokens = JSON.parse(localStorage.getItem('eduwills_tokens') || '[]');
    const found = tokens.find((t: { token: string; expiresAt: string }) => t.token === clean && new Date(t.expiresAt).getTime() > Date.now());
    if (!found) {
    ...
    const current = localStorage.getItem('eduwills_current_user');
    const users = JSON.parse(localStorage.getItem('eduwills_users') || '[]');
    const updated = users.map((u: Record<string, unknown>) => u.username === current ? { ...u, activated: true, activationExpiresAt: found.expiresAt } : u);
    localStorage.setItem('eduwills_users', JSON.stringify(updated));
    setTokenMessage(`Account activated until ${new Date(found.expiresAt).toLocaleDateString()}.`);
  };
    ...
    <main className="min-h-screen bg-paper px-4 py-5 pb-10 sm:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-6 rounded-[2rem] bg-ink p-7 text-white sm:p-9">
          ...
          <h1 className="mt-3 text-3xl font-black">Hello {displayName || 'Learner'}, let's activate your account.</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Your username is already attached to this page, so you don't need to remember or retype it when sending your payment proof.</p>
        </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_390px]">
          <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-soft sm:p-9">
            ...
            <div className="mt-6 rounded-3xl border border-slate-200 p-6">
              ...
              <div className="mt-5 rounded-2xl bg-amber-50 p-5"><div className="text-sm font-black text-amber-900">What to send</div><ol className="mt-3 list-decimal space-y-2 pl-5 text-xs leading-5 text-amber-900/80"><li>Make the activation payment to the account above.</li><li>Keep a clear receipt showing the transaction details.</li><li>Send the receipt/proof of payment.</li><li>Send the amount paid and payment date/time.</li><li>Your EDUWILLS username will be included automatically.</li></ol></div>
          ...
          <aside>
            <div className="rounded-[2rem] border border-slate-200 bg-white p-7 shadow-soft">
              <div className="grid h-12 w-12 place-items-center rounded-2xl bg-emerald-50 text-emerald-600"><KeyRound size={22} /></div>
              <h2 className="mt-5 text-xl font-black">Have a WilliToken?</h2>
              <p className="mt-2 text-sm leading-6 text-slate-500">Enter the letters-and-numbers token given to you by the admin.</p>
              <input value={token} onChange={(e) => setToken(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ''))} className="mt-5 w-full rounded-xl border border-slate-200 bg-paper px-4 py-3 font-mono uppercase tracking-widest outline-none focus:border-eduBlue" placeholder="AB12CD34EF" />
              <button type="button" onClick={activateToken} className="mt-3 w-full rounded-xl bg-ink px-5 py-3.5 font-black text-white">Activate with WilliToken</button>
              {tokenMessage && <p className="mt-3 text-xs font-semibold text-slate-600">{tokenMessage}</p>}
              <div className="mt-4 flex gap-2 text-xs leading-5 text-slate-400"><CheckCircle2 size={15} /> Token duration is set by the admin.</div>
            </div>
