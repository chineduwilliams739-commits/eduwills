...
const USD_RATES: Record<string, number> = { USD: 1, GBP: .75, EUR: .86, GHS: 11.2, KES: 129, ZAR: 17.8, XOF: 560, NGN: 1550 };
...
type Quote = { localCurrency: string; localAmount: number; paymentCurrency: string; paymentAmount: number; international: boolean };
...
function quote(categories: string[], currency: string, country: string): Quote { const ngn = categories.reduce((sum, c) => sum + (PRICES[c] || 0), 0); if (country === 'NG' && currency === 'NGN') return { localCurrency: 'NGN', localAmount: ngn, paymentCurrency: 'NGN', paymentAmount: ngn, international: false }; const usd = ngn / NGN_PER_USD + 1; const local = usd * (USD_RATES[currency] || 1); return { localCurrency: currency, localAmount: Math.round(local * 100) / 100, paymentCurrency: 'USD', paymentAmount: Math.round(usd * 100) / 100, international: true }; }
