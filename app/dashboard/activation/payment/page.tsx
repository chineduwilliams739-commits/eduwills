...
const BASE = '/eduwills';
const FUNCTION_URL = 'https://us-central1-eduwills.cloudfunctions.net/paystackInitialize';
const PUBLIC_KEY = process.env.NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY || '';
const NGN_AMOUNT = 400000; // ₦4,000 in kobo
const USD_AMOUNT = 500; // $5.00 in cents
...
export default function ActivationPaymentPage() {
  ...
  async function waitForActivation() {
    for (let i = 0; i < 12; i += 1) {
      ...
      if (data?.activationStatus === 'active' && data?.activationExpiresAt) { setActivated(true); setMessage('Payment confirmed and your EduWills activation has been applied.'); return true; }
    }
    return false;
  }
  ...
  async function pay() {
    ...
    try {
      ...
      const response = await fetch(FUNCTION_URL, { method: 'POST', headers: { Authorization: `Bearer ${idToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ amount, currency, durationMs: DURATION_MS, categories: ['book'] }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data?.detail || data?.error || 'Payment initialization failed.');
      ...
      if (!success) setMessage('Payment was initiated, but activation is still being confirmed. Please wait a little and refresh your dashboard.');
    } catch (e: any) { setMessage(e?.message || 'Could not start payment.'); }
...
<div className="mt-7 grid gap-3 sm:grid-cols-2"><button onClick={()=>setCurrency('NGN')} className={`rounded-2xl border p-5 text-left ${currency==='NGN'?'border-cyan-400 bg-cyan-400/10':'border-white/10 bg-white/5'}`}><p className="text-xs font-bold uppercase tracking-wider text-slate-500">Nigeria</p><p className="mt-1 text-2xl font-black">₦4,000</p><p className="mt-1 text-xs text-slate-400">30-day Book Learner activation</p></button><button onClick={()=>setCurrency('USD')} className={`rounded-2xl border p-5 text-left ${currency==='USD'?'border-cyan-400 bg-cyan-400/10':'border-white/10 bg-white/5'}`}><p className="text-xs font-bold uppercase tracking-wider text-slate-500">International</p><p className="mt-1 text-2xl font-black">$5</p><p className="mt-1 text-xs text-slate-400">30-day Book Learner activation</p></button></div>
...
<div className="mt-6 grid gap-3 sm:grid-cols-2"><div className="flex gap-3 rounded-xl bg-white/5 p-4"><ShieldCheck className="shrink-0 text-emerald-400" size={18}/><p className="text-xs leading-5 text-slate-400">Your Paystack secret key is never sent to the browser. The server initializes and verifies payments.</p></div><div className="rounded-xl bg-white/5 p-4"><p className="text-xs leading-5 text-slate-400">Successful payments automatically generate and assign a WilliToken through the verified webhook.</p></div></div></section></div></main>;
}
