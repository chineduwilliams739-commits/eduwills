...
type User = {
  id: string; uid?: string; fullName?: string; username?: string; phone?: string; activated?: boolean;
  activationExpiresAt?: any; category?: string; categories?: string[]; educationLevel?: string;
  educationLevels?: string[]; schoolLevel?: string; schoolLevels?: string[];
};
...
export default function AdminPage() {
  ...
  useEffect(() => onAuthStateChanged(auth, async user => {
    ...
    try {
      ...
      await load();
    } catch (e: any) { setError(e?.code === 'permission-denied' ? 'Firebase denied Admin access.' : 'Could not load Admin data.'); setLoading(false); }
  }), []);
  ...
  const exportUsers = () => {
    const rows = [['Name', 'Username', 'Phone', 'Categories', 'Activated', 'Books', 'WilliToken expiry']];
    users.forEach(u => { const latest = userTokens(u.uid || u.id)[0]; rows.push([u.fullName || '', u.username ? `@${u.username}` : '', u.phone || '', categoriesFor(u).join(' | ') || 'Not set', u.activated ? 'Yes' : 'No', String(userBooks(u.uid || u.id).length), formatExpiry(tokenExpiry(latest))]); });
  ...
  return <main className="min-h-screen overflow-y-auto bg-slate-950 px-4 py-6 text-white sm:px-6"><div className="mx-auto max-w-7xl pb-12">
    ...
    {tab === 'users' && <section className="mt-5 rounded-3xl border border-white/10 bg-white/5 p-5 sm:p-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.18em] text-cyan-300">Users</p><h2 className="mt-1 text-2xl font-black">All EDUWILLS Users</h2><p className="mt-1 text-sm text-slate-400">Total users: <strong className="text-white">{users.length}</strong></p></div><button onClick={exportUsers} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-slate-900 px-4 py-2.5 text-sm font-bold"><Download size={16}/> Export users</button></div>
      ...
      <div className="mt-4 max-h-[540px] overflow-y-auto rounded-2xl border border-white/10 p-2"><div className="space-y-2">{filteredUsers.map(u => { const uid = u.uid || u.id; const cats = categoriesFor(u); const latest = userTokens(uid)[0]; const exp = tokenExpiry(latest); return <button key={u.id} onClick={() => { setSelectedUserId(uid); setGenerated(''); }} className={`block w-full rounded-2xl border p-4 text-left ${selectedUserId === uid ? 'border-cyan-300/60 bg-cyan-400/10' : 'border-white/10 bg-slate-900/70'}`}><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="font-black">{u.fullName || 'Unnamed user'}</p><p className="text-xs text-slate-400">{u.username ? `@${u.username}` : 'No username'}{u.phone ? ` · ${u.phone}` : ''}</p><div className="mt-2 flex flex-wrap gap-1.5">{(cats.length ? cats : ['Category not set']).map(c => <span key={c} className="rounded-full bg-violet-400/10 px-2.5 py-1 text-[11px] font-black text-violet-200">{c}</span>)}</div></div><div className="text-right text-xs"><div><span className="rounded-full bg-cyan-400/10 px-2.5 py-1 font-bold text-cyan-300">{userBooks(uid).length} books</span></div><div className="mt-2 text-slate-400">{exp ? `WilliToken expires ${formatExpiry(exp)}` : 'No active WilliToken'}</div>{exp && <div className="mt-1 text-emerald-300">{remaining(exp)}</div>}</div></div></button>; })}</div>{!filteredUsers.length && <p className="py-10 text-center text-sm text-slate-500">No users match this search.</p>}</div>
      {selectedUser && <div className="mt-5 rounded-3xl border border-cyan-300/20 bg-cyan-400/5 p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.18em] text-cyan-300">Selected user</p><h3 className="mt-1 text-xl font-black">{selectedUser.fullName || 'Unnamed user'}</h3><p className="text-sm text-slate-400">{selectedUser.username ? `@${selectedUser.username}` : 'No username'}{selectedUser.phone ? ` · ${selectedUser.phone}` : ''}</p><div className="mt-2 flex flex-wrap gap-1.5">{(categoriesFor(selectedUser).length ? categoriesFor(selectedUser) : ['Category not set']).map(c => <span key={c} className="rounded-full bg-violet-400/10 px-2.5 py-1 text-xs font-black text-violet-200">{c}</span>)}</div></div></div><div className="mt-5 border-t border-white/10 pt-5"><h4 className="flex items-center gap-2 font-black"><KeyRound size={18}/> WilliToken</h4><div className="mt-3 grid gap-3 sm:grid-cols-[1fr_auto]"><select value={duration} onChange={e => setDuration(e.target.value)} className="rounded-xl border border-white/10 bg-slate-900 p-3 text-sm font-bold">{durations.map(x => <option key={x[0]} value={x[0]}>{x[0]}</option>)}</select><button onClick={createToken} className="rounded-xl bg-cyan-400 px-5 py-3 text-sm font-black text-slate-950">Generate WilliToken</button></div><p className="mt-2 text-xs text-slate-400">If this user has category expiry policies configured, the shortest applicable category policy overrides the dropdown. Otherwise the selected duration is used.</p>{generated && <div className="mt-3 flex items-center gap-2 rounded-2xl bg-emerald-400/10 p-4"><code className="flex-1 break-all font-black tracking-widest">{generated}</code><button onClick={copy} className="rounded-lg border border-white/10 p-2">{copied ? <Check size={16}/> : <Copy size={16}/>}</button></div>}</div><div className="mt-5"><p className="text-xs font-black uppercase tracking-[.16em] text-slate-400">Active token expiry</p><div className="mt-2 space-y-2">{userTokens(selectedUser.uid || selectedUser.id).map(t => { const exp = tokenExpiry(t); return <div key={t.id} className="rounded-2xl border border-white/10 bg-slate-950/60 p-3"><div className="flex flex-wrap justify-between gap-2"><code className="font-bold">{t.token || t.id}</code><span className={exp && exp.getTime() > Date.now() ? 'text-emerald-300' : 'text-red-300'}>{remaining(exp)}</span></div><p className="mt-1 text-xs text-slate-400">Categories at issue: {(t.categories || categoriesFor(selectedUser)).join(', ') || 'Not set'} · Expires: {formatExpiry(exp)}</p></div>; })}{!userTokens(selectedUser.uid || selectedUser.id).length && <p className="text-sm text-slate-500">No active WilliToken for this user.</p>}</div></div></div>}
    </section>}
