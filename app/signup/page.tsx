...
export default function SignUpPage() {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await setDoc(doc(db, 'phoneIndex', phone), { uid: credential.user.uid, authEmail: email, username, phoneE164: phone });
      await setDoc(doc(db, 'emailIndex', email), { uid: credential.user.uid, username, phoneE164: phone });
      localStorage.setItem('eduwills_current_user', username);
      localStorage.setItem('eduwills_current_uid', credential.user.uid);
      window.location.href = `${BASE}/verify-email/`;
    } catch (error: unknown) {
      const code = error instanceof Error ? error.message : '';
      if (code.includes('email-already-in-use')) setMessage('That email address is already registered.');
    ...
    <main className="min-h-screen bg-paper px-4 py-5 sm:px-6 sm:py-8">
      ...
      <div className="mx-auto w-full max-w-5xl">
        ...
        <div className="mx-auto mt-6 grid w-full max-w-5xl overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-soft sm:mt-8 sm:rounded-[2rem] lg:grid-cols-[.82fr_1.18fr]">
          <aside className="hidden bg-ink p-10 text-white lg:block"><div className="text-2xl font-black">EDUWILLS</div><div className="mt-16 max-w-sm"><div className="text-xs font-black uppercase tracking-[.2em] text-cyan-200">Create your learning account</div><h1 className="mt-4 text-4xl font-black leading-tight">Start free. Activate when you're ready.</h1><p className="mt-5 leading-7 text-slate-300">Registration is free. Quiz, History and EDUWILLS AI become available after activation.</p></div><div className="mt-12 space-y-4 text-sm text-slate-300">{['Free account creation','Email verification only','Activate later with WilliToken'].map((item)=><div key={item} className="flex items-center gap-3"><span className="grid h-6 w-6 place-items-center rounded-full bg-white/10 text-cyan-200"><Check size={14}/></span>{item}</div>)}</div></aside>
          <section className="min-w-0 p-5 sm:p-8 md:p-10"><div className="mx-auto w-full max-w-xl"><p className="text-xs font-black uppercase tracking-[.2em] text-eduBlue">Free registration</p><h2 className="mt-2 text-3xl font-black tracking-tight text-ink">Join EDUWILLS</h2><p className="mt-2 text-sm leading-6 text-slate-500">Use a real email address so we can verify your account and recover it if you forget your password.</p>
            <form className="mt-7 space-y-5" onSubmit={submit}>
              ...
              <label className="block text-sm font-bold text-ink">Email address<input name="email" required type="email" autoComplete="email" className="mt-2 block w-full rounded-xl border border-slate-200 bg-paper px-4 py-3 outline-none focus:border-eduBlue" placeholder="you@example.com"/><span className="mt-1 block text-xs font-normal text-slate-400">A verification link will be sent here. No SMS verification is required.</span></label>
              <label className="block text-sm font-bold text-ink">Phone number<div className="eduwills-phone mt-2 overflow-hidden rounded-xl border border-slate-200 bg-paper"><PhoneInput international defaultCountry="NG" countryCallingCodeEditable={false} value={phone} onChange={setPhone} placeholder="Enter your phone number"/></div><span className="mt-1 block text-xs font-normal text-slate-400">Select your country from the flag/country-code selector. The full international number is stored.</span></label>
              ...
              <div className="flex items-start gap-3 rounded-2xl bg-slate-50 p-4"><ShieldCheck className="mt-0.5 shrink-0 text-emerald-600" size={18}/><p className="text-xs leading-5 text-slate-500">Your account is created for free. Email verification is used for account security and recovery. Your phone number is collected for your profile and future account services, but no verification SMS is sent.</p></div>
