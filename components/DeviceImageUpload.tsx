...
async function compressImage(file:File){
 ...
 const blob=await new Promise<Blob|null>(resolve=>canvas.toBlob(resolve,'image/webp',.65));
 if(!blob)return file;return new File([blob],'image.webp',{type:'image/webp',lastModified:Date.now()});
}
export default function DeviceImageUpload({path,onUploaded,label='Upload image',uid}:{path:string;onUploaded:(url:string)=>void;label?:string;uid:string}){
 ...
 async function choose(e:React.ChangeEvent<HTMLInputElement>){const file=e.target.files?.[0];e.target.value='';if(!file)return;setMessage('');if(file.size>15*1024*1024){setMessage('Image must be smaller than 15 MB.');return}setBusy(true);setProgress(3);try{
  const optimized=await compressImage(file);setProgress(20);const safeName=optimized.name.replace(/[^a-zA-Z0-9._-]/g,'_').slice(-60)||'image.webp';
  const storageRef=ref(storage,`${path}/${uid}/${Date.now()}_${safeName}`);let url:string;
  if(optimized.size<=2*1024*1024){await uploadBytes(storageRef,optimized,{contentType:optimized.type,customMetadata:{uploadedBy:uid,source:'device'}});setProgress(92);url=await getDownloadURL(storageRef)}else{const task=uploadBytesResumable(storageRef,optimized,{contentType:optimized.type,customMetadata:{uploadedBy:uid,source:'device'}});await new Promise<void>((resolve,reject)=>task.on('state_changed',s=>setProgress(20+Math.round(s.bytesTransferred/s.totalBytes*72)),()=>reject(new Error('Image upload failed. Please try again.')),()=>resolve()));url=await getDownloadURL(storageRef)}
