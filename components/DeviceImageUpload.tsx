...
function uploadFile(storageRef:ReturnType<typeof ref>,file:File,onProgress:(value:number)=>void){
 return new Promise<void>((resolve,reject)=>{
  let finished=false;
  const timer=setTimeout(()=>{if(finished)return;finished=true;try{task.cancel()}catch{};reject(Object.assign(new Error('IMAGE_UPLOAD_TIMEOUT'),{code:'storage/retry-limit-exceeded'}))},60000);
  onProgress(2);
  const task=uploadBytesResumable(storageRef,file,{contentType:file.type,customMetadata:{source:'device'}});
  task.on('state_changed',snap=>{if(!finished)onProgress(Math.min(95,Math.max(2,Math.round((snap.bytesTransferred/snap.totalBytes)*95))))},error=>{if(finished)return;finished=true;clearTimeout(timer);reject(error)},()=>{if(finished)return;finished=true;clearTimeout(timer);onProgress(96);resolve()});
 });
}
...
export default function DeviceImageUpload({path,onUploaded,label='Upload image',uid}:{path:string;onUploaded:(url:string)=>void;label?:string;uid:string}){
 ...
 async function choose(e:React.ChangeEvent<HTMLInputElement>){
  ...
  setBusy(true);setProgress(1);setMessage('Preparing secure upload…');
  try{
   ...
   for(let attempt=1;attempt<=3&&!uploaded;attempt++){
    try{await uploadFile(storageRef,optimized,setProgress);uploaded=true}catch(error:any){lastError=error;const code=String(error?.code||'');if(code!=='storage/retry-limit-exceeded'||attempt===3)throw error;setMessage(`Connection interrupted. Retrying upload (${attempt}/3)…`);await new Promise(r=>setTimeout(r,700*attempt));}
   }
   ...
   const detail=code==='storage/unauthorized'?'Firebase denied this image upload. Please try again.':code==='storage/unauthenticated'?'Your session expired. Please sign in again.':code==='storage/canceled'?'Upload canceled. Please try again.':code==='storage/retry-limit-exceeded'?'The connection kept failing. Please try again on a stronger connection.':e?.message||'Image upload failed. Please try again.';
   setMessage(detail);
 ...
 return <div>
  ...
  {!busy&&message&&/failed|again|expired|authorized|large|too large|connection|session|denied/i.test(message)&&<button type="button" onClick={()=>inputRef.current?.click()} className="mt-2 inline-flex items-center gap-1.5 text-[11px] font-black text-cyan-700"><RefreshCw size={13}/> Try another image</button>}
 </div>;
}
