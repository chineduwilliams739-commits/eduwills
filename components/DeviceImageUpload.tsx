...
async function compressImage(file:File){
 ...
 const bitmap=await createImageBitmap(file);const max=768;const scale=Math.min(1,max/Math.max(bitmap.width,bitmap.height));
 const w=Math.max(1,Math.round(bitmap.width*scale)),h=Math.max(1,Math.round(bitmap.height*scale));const canvas=document.createElement('canvas');canvas.width=w;canvas.height=h;const ctx=canvas.getContext('2d');if(!ctx)throw new Error('Image processing is unavailable on this device.');ctx.drawImage(bitmap,0,0,w,h);bitmap.close();
 const blob=await new Promise<Blob|null>(resolve=>canvas.toBlob(resolve,'image/webp',.55));if(!blob||blob.size>=file.size)return file;return new File([blob],'image.webp',{type:'image/webp',lastModified:Date.now()});
}
export default function DeviceImageUpload({path,onUploaded,label='Upload image',uid}:{path:string;onUploaded:(url:string)=>void;label?:string;uid:string}){
 ...
 async function choose(e:React.ChangeEvent<HTMLInputElement>){const file=e.target.files?.[0];e.target.value='';if(!file)return;setMessage('');if(file.size>12*1024*1024){setMessage('Image must be smaller than 12 MB.');return}setBusy(true);setProgress(2);try{
  const optimized=await compressImage(file);setProgress(25);if(optimized.size>2*1024*1024)throw new Error('Please choose a smaller image (2 MB maximum after compression).');
  const safeName=optimized.name.replace(/[^a-zA-Z0-9._-]/g,'_').slice(-60)||'image.webp';const storagePath=path==='users'?`users/${uid}/profile/${Date.now()}_${safeName}`:`${path}/${uid}/${Date.now()}_${safeName}`;const storageRef=ref(storage,storagePath);setProgress(35);
  const upload=uploadBytes(storageRef,optimized,{contentType:optimized.type,customMetadata:{uploadedBy:uid,source:'device'}});const result=await Promise.race([upload,new Promise<never>((_,reject)=>setTimeout(()=>reject(new Error('Image upload timed out. Please check your connection and try again.')),30000))]);void result;setProgress(90);const url=await getDownloadURL(storageRef);setProgress(100);onUploaded(url);setMessage('Image ready.');
