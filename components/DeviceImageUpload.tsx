...
export default function DeviceImageUpload({path,onUploaded,label='Upload image',uid}:{path:string;onUploaded:(url:string)=>void;label?:string;uid:string}){
 ...
 async function choose(e:React.ChangeEvent<HTMLInputElement>){const file=e.target.files?.[0];e.target.value='';if(!file)return;setMessage('');if(file.size>12*1024*1024){setMessage('Image must be smaller than 12 MB.');return}setBusy(true);setProgress(2);try{
  const optimized=await compressImage(file);setProgress(18);
  ...
  if(optimized.size<=2*1024*1024){
   const upload=uploadBytes(storageRef,optimized,{contentType:optimized.type,customMetadata:{uploadedBy:uid,source:'device'}});
   const result=await Promise.race([upload,new Promise<never>((_,reject)=>setTimeout(()=>reject(new Error('Image upload timed out. Please check your connection and try again.')),30000))]);
   void result;setProgress(90);url=await getDownloadURL(storageRef);
  }else{
   ...
   await new Promise<void>((resolve,reject)=>{let settled=false;const finish=(fn:(value?:any)=>void,value?:any)=>{if(settled)return;settled=true;window.clearTimeout(timer);fn(value)};const timer=window.setTimeout(()=>{try{task.cancel()}catch{};finish(reject,new Error('Image upload timed out. Please check your connection and try again.'))},30000);task.on('state_changed',snap=>{const pct=snap.totalBytes?Math.round((snap.bytesTransferred/snap.totalBytes)*65):0;setProgress(Math.min(90,25+pct));},err=>finish(reject,err),()=>finish(resolve))});
