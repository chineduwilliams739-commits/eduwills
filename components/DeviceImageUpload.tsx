...
export default function DeviceImageUpload({path,onUploaded,label='Upload image',uid}:{path:string;onUploaded:(url:string)=>void;label?:string;uid:string}){
 ...
 async function choose(e:React.ChangeEvent<HTMLInputElement>){const file=e.target.files?.[0];e.target.value='';if(!file)return;setMessage('');if(file.size>15*1024*1024){setMessage('Image must be smaller than 15 MB.');return}setBusy(true);setProgress(5);try{
  const optimized=await compressImage(file);setProgress(15);const safeName=optimized.name.replace(/[^a-zA-Z0-9._-]/g,'_').slice(-60)||'image.webp';
  const storageRef=ref(storage,`${path}/${uid}/${Date.now()}_${safeName}`);const task=uploadBytesResumable(storageRef,optimized,{contentType:optimized.type,customMetadata:{uploadedBy:uid,source:'device'}});
  await new Promise<void>((resolve,reject)=>task.on('state_changed',s=>setProgress(15+Math.round(s.bytesTransferred/s.totalBytes*80)),()=>reject(new Error('Image upload failed. Please try again.')),()=>resolve()));
  const url=await getDownloadURL(storageRef);onUploaded(url);setProgress(100);setMessage('Ready. EDUWILLS safety moderation will remove prohibited media.');
