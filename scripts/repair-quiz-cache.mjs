...
if(!startRe.test(page))throw new Error('start function not found');
const startReplacement=` async function start(){
  ...
  try{
   ...
   if(cached&&cached.length>=questions){
    ...
    const ref=await addDoc(collection(db,'quizHistory'),{userId:auth.currentUser.uid,...setupData,status:'ready',score:null,percentage:null,questionsData:cached,total:cached.length,generationMode:'cached-question-pool',createdAt:serverTimestamp()});
    const s:Setup={id:ref.id,...setupData};setSetup(s);setIdx(0);setDone(false);setConfirm(false);setExitConfirm(false);setAnswers(Array(cached.length).fill(null));answersRef.current=Array(cached.length).fill(null);setElapsed(0);setSeconds(s.duration?s.duration*60:null);setQs(cached);setRemarks('');setGenerationStatus('');setLeaveAttempts(0);leaveAttemptsRef.current=0;allowNavigationRef.current=false;historyArmedRef.current=false;quizUrlRef.current=window.location.href;return;
   }
