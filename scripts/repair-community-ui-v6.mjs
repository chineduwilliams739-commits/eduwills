...
group=group.replace(
  ...
  'onInput={e=>setDraft((e.target as HTMLInputElement).value)} onKeyDown='
);
if(!group.includes('GROUP_MESSAGE_INPUT_HARDENING')){
  group=group.replace(
    'return <main className="min-h-screen bg-[#eef3f7] text-ink">',
    'return <main className="min-h-screen bg-[#eef3f7] text-ink"><style id="GROUP_MESSAGE_INPUT_HARDENING">{`textarea[aria-label="Write a message"]{writing-mode:horizontal-tb !important;text-orientation:mixed !important;direction:ltr !important;white-space:pre-wrap !important;unicode-bidi:plaintext !important;}`}</style>'
  );
}
const oldInfo = "<div className=\"mt-4 rounded-2xl bg-slate-50 p-4\"><p className=\"text-sm leading-6 text-slate-600\">{g.description||'A focused EDUWILLS learning community.'}</p><p className=\"mt-3 text-xs font-semibold text-slate-500\">Use MEMBERS to view everyone in this group.</p></div>";
const newInfo = "<div className=\"mt-4 space-y-3\"><div className=\"rounded-2xl bg-slate-50 p-4\"><p className=\"text-sm leading-6 text-slate-600\">{g.description||'A focused EDUWILLS learning community.'}</p><p className=\"mt-3 text-xs font-semibold text-slate-500\">Use MEMBERS to view everyone in this group.</p></div><div className=\"rounded-2xl border border-cyan-100 bg-cyan-50/60 p-4\"><div className=\"flex items-center gap-2\"><Shield size={16} className=\"text-cyan-700\"/><p className=\"text-sm font-black text-cyan-950\">Rules & regulations</p></div><div className=\"mt-3 space-y-2\">{rules.map((r,i)=><div key={r[0]} className=\"rounded-xl bg-white/80 p-3\"><p className=\"text-xs font-black text-slate-800\">{i+1}. {r[0]}</p><p className=\"mt-1 text-[11px] leading-5 text-slate-600\">{r[1]}</p></div>)}</div></div></div>";
if(!group.includes('Rules & regulations')){
  if(!group.includes(oldInfo)) throw new Error('GROUP_UI_FIX_INFO_ANCHOR_MISSING');
  group=group.replace(oldInfo,newInfo);
}
...
const lockControl = `<section className="rounded-2xl border border-amber-200 bg-amber-50/60 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-wider text-amber-700">MESSAGE CONTROL</p><p className="mt-1 text-sm font-black text-slate-900">LOCK SENDING</p><p className="mt-1 text-[11px] leading-5 text-slate-600">When enabled, members can still read the group but only admins can send messages.</p></div><button type="button" onClick={async()=>{try{const next=!isLocked;await updateDoc(doc(db,"communityGroups",id),{messagingLocked:next,updatedAt:serverTimestamp()});setIsLocked(next);setG((x:any)=>x?{...x,messagingLocked:next}:x);setNotice(next?"Message sending is locked for members.":"Message sending is unlocked for members.")}catch(e:any){setNotice(e?.message||"Could not change message lock.")}}} className={'relative h-8 w-14 rounded-full transition '+(isLocked?'bg-amber-600':'bg-slate-300')} aria-label="LOCK SENDING"><span className={'absolute top-1 h-6 w-6 rounded-full bg-white shadow transition '+(isLocked?'left-7':'left-1')}/></button></div><p className="mt-3 text-[10px] font-black uppercase tracking-wider text-amber-800">{isLocked?'LOCKED':'UNLOCKED'}</p></section>`;
...
group=group.replace(
  ...
  "async function send(){if((isLocked&&!isAdmin)||((!draft.trim()&&!image)||!user))return;"
);
group=group.replace(
  '<button onClick={send} disabled={!draft.trim()&&!image} className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-ink text-white disabled:opacity-30"><Send size={17}/></button>',
  '<button onClick={send} disabled={(isLocked&&!isAdmin)||(!draft.trim()&&!image)} title={isLocked&&!isAdmin?\'Sending is locked by an admin\':\'Send message\'} className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-ink text-white disabled:opacity-30"><Send size={17}/></button>'
);
...
const required=[
  ...
  ['horizontal input hardening','GROUP_MESSAGE_INPUT_HARDENING'],
  ['rules in info','Rules & regulations'],
  ['lock control','LOCK SENDING'],
  ['lock field','messagingLocked'],
  ['message lock enforcement','isLocked&&!isAdmin'],
];
...
console.log('Community UI v6 requested follow-up fixes applied.');
