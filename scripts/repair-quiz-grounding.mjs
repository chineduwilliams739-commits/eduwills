...
const verifiedBookResearch = {
  'sanya|oyin olugbile': [
    ...
    'VERIFIED SOURCES: Oyin Olugbile official book page; Masobe Books; National Library of Nigeria; Nigeria Prize for Literature materials.',
  ...
  'scars|gen leo irabor': 'VERIFIED BOOK: SCARS: Nigeria’s Journey and the Boko Haram Conundrum. Use the exact selected title and author; never substitute a similarly named work.',
  'scars nigeria s journey and the boko haram conundrum|gen leo irabor': 'VERIFIED BOOK: SCARS: Nigeria’s Journey and the Boko Haram Conundrum. Use the exact selected title and author; never substitute a similarly named work.'
};

const verifiedCode = [
  'const VERIFIED_BOOK_RESEARCH:Record<string,string>=' + JSON.stringify(verifiedBookResearch) + ';',
...
const guardCode = [
  ...
  "  const exact=books.some(b=>norm(b.title)==='sanya'&&norm(b.author)==='oyin olugbile');",
  '  if(exact){',
  "    const bad=/\\b(?:sanya|the protagonist|the main character)\\b[\\s\\S]{0,220}\\b(?:he|him|his|boy|male)\\b/i.test(String(q.question))||/\\b(?:he|him|his|boy|male)\\b[\\s\\S]{0,220}\\b(?:sanya|the protagonist|the main character)\\b/i.test(String(q.question));",
  '    if(bad)return false;',
  "    if(/\\b(?:medical doctor|journalist|journalism|military officer|architect|architecture|rural clinic|modern housing estates|cryptocurrency)\\b/.test(text))return false;",
  '  }',
  "  const evidenceWords=norm(research).split(' ').filter(w=>w.length>=5).filter(w=>!['about','there','which','their','would','could','these','those','story','book','author','verified','sources'].includes(w));",
  '  if(evidenceWords.length>=8){const overlap=evidenceWords.filter(w=>text.includes(w)).slice(0,40).length;if(overlap===0)return false;}',
  '  return true;',
  '}',
  ''
].join('\n');
