...
// Do not depend on one exact formatting of the activation page. Earlier repairs
// intentionally compacted the component declaration onto one line.
...
// Replace the old payment confirmation presentation when present. This regex is
// deliberately scoped to the final paymentSuccess block before the section closes.
...
if (paymentBlock.test(source)) {
  source = source.replace(paymentBlock, newPaymentBlock);
} else if (!source.includes('Your WilliToken')) {
  throw new Error('Payment success UI marker not found.');
}
