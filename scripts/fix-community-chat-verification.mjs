...
s = s.replace(
  /const recentRows=useMemo\(\(\)=>\[\.\.\.chatRows,\.\.\.groupRows\]\.sort\(\(a:any,b:any\)=>\(b\.updatedAt\|\|0\)-(a\.updatedAt\|\|0\)\),\[chatRows,groupRows\]\);/,
...
// fallback scan. Mark it explicitly so the source verification can validate the
// actual behavior without depending on punctuation such as a Unicode ellipsis.
