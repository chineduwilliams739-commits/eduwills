...
# Keep incomplete profiles visible to Admin without pretending the Firebase UID is a username.
src = src.replace("username?: string; phone?: string;", "username?: string; email?: string; phone?: string;")
src = src.replace("return u?.fullName || (u?.username ? `@${u.username}` : uid);", "return u?.fullName || (u?.username ? `@${u.username}` : (u?.email || 'Profile incomplete'));" )
src = src.replace("{u.username ? '@' + u.username : id}", "{u.username ? '@' + u.username : (u.email || 'Profile incomplete')}")
src = src.replace("{u.fullName || u.username || u.uid || u.id}", "{u.fullName || u.username || u.email || 'Profile incomplete'}")
...
if 'tokenUserSearch' not in src:
    ...
    picker = '''              <label className="mt-5 block text-xs font-black uppercase text-slate-400">Find user to generate WilliToken</label>\n              <div className="relative mt-2">\n                <Search size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />\n                <input value={tokenUserSearch} onChange={e => setTokenUserSearch(e.target.value)} placeholder="Search name, username, phone or UID…" className="w-full rounded-xl border border-white/10 bg-slate-900 py-3 pl-9 pr-3 text-sm outline-none focus:border-cyan-300/50" />\n              </div>\n              {tokenUserSearch.trim() && (\n                <div className="mt-2 max-h-56 space-y-2 overflow-y-auto">\n                  {tokenUserMatches.length ? tokenUserMatches.map(u => {\n                    const id = u.uid || u.id;\n                    const selected = selectedUid === id;\n                    return (\n                      <button key={u.id} type="button" onClick={() => { setSelectedUid(id); setTokenUserSearch(u.fullName || (u.username ? '@' + u.username : (u.email || 'Profile incomplete'))); }} className={'w-full rounded-xl border p-3 text-left transition ' + (selected ? 'border-cyan-300 bg-cyan-400/15 ring-1 ring-cyan-300/50' : 'border-white/10 bg-slate-900 hover:bg-white/10')}>\n                        <div className="flex items-center justify-between gap-3">\n                          <div><p className="font-black">{u.fullName || 'Unnamed user'}</p><p className="text-xs text-slate-400">{u.username ? '@' + u.username : (u.email || 'Profile incomplete')}</p></div>\n                          {selected && <Check size={17} className="text-cyan-300" />}\n                        </div>\n                      </button>\n                    );\n                  }) : <p className="p-3 text-sm text-slate-400">No matching users found.</p>}\n                </div>\n              )}\n              {selectedUser && <p className="mt-2 text-xs font-bold text-cyan-200">Selected: {selectedUser.fullName || selectedUser.username || selectedUser.email || 'Profile incomplete'}</p>}\n\n              <label className="mt-4 block text-xs font-black uppercase text-slate-400">User</label>\n              <select value={selectedUid} onChange={e => setSelectedUid(e.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900 px-3 py-3 text-sm font-bold"><option value="">Select user…</option>{users.map(u => <option key={u.id} value={u.uid || u.id}>{u.fullName || u.username || u.email || 'Profile incomplete'}</option>)}</select>'''
...
print('Admin WilliToken user-picker, expiry cleanup and safe profile fallback applied.')
