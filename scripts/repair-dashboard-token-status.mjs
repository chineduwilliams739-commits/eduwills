...
const dashboardPattern = /\n<div className="mt-7 grid items-start gap-4 sm:grid-cols-2 lg:grid-cols-5">[\s\S]*?<\/div><\/div><nav className="fixed bottom-0/;
const dashboardMatch = dashboard.match(dashboardPattern);
if (!dashboardMatch) throw new Error('Dashboard navigation grid block not found');
