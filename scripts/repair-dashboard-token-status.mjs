...
const dashboardPattern = /\n<div className="mt-7 grid items-start gap-4 sm:grid-cols-2 lg:grid-cols-[45]">[\s\S]*?<\/div><\/div><nav className="fixed bottom-0/;
if (!dashboardPattern.test(dashboard)) throw new Error('Dashboard navigation grid block not found after previous repair.');
