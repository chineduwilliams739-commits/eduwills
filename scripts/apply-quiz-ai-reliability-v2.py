...
# Prevent any attempt to generate a quiz without a real selected saved book.
...
# Make generation failures propagate so a failed generation is never counted as a successful free quiz.
...
# Make the in-test interface more polished and easier to scan without changing quiz behavior.
...
# The existing Gemini instance is JSON-only for quiz generation. Do not use that
# instance for chat: a JSON response MIME type can cause ordinary AI answers to fail.
...
# Generate larger quiz batches to reduce serial AI round trips.
...
# generateQuiz must reject empty input rather than silently inventing a placeholder book.
...
# Hard validation: the patch must have produced real source, not escaped JSX.
