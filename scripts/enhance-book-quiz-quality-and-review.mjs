...
if (!page.includes('Review your quiz before generation') && page.includes(reviewMarker)) {
  const reviewLines = [
    '  /* ------------------------------------------------------------------------ */',
    ...
    '            <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs font-black uppercase tracking-wider text-cyan-200"><Sparkles size={13} /> Quiz preparation</div>',
    ...
    '                <div><p className="text-xs font-black uppercase tracking-wider text-cyan-200">Estimated time to start</p><p className="mt-1 text-lg font-black">Up to {estimatedMinutes} minute{estimatedMinutes === 1 ? \'\' : \'s\'}</p><p className="mt-1 text-sm leading-6 text-slate-300">This is an estimate, capped at 10 minutes. Generation time depends on question count, selected books and live book research. Your quiz timer starts only after the requested questions are ready.</p></div>',
    '              </div>',
    '            </div>',
