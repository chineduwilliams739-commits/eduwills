...
quiz = quiz.replace(/  function makeResultImage\(\): Promise<Blob> \{[\s\S]*?\n  \}\n  async function downloadResult/, `  function makeResultImage(): Promise<Blob> {
    return new Promise((resolve) => {
      ...
      g.font = '900 116px sans-serif'; g.fillText(\`${pct}%\`, 75, 350); g.font = '700 34px sans-serif'; g.fillText(\`${score}/${qs.length} correct\`, 80, 405);
      g.font = '600 25px sans-serif';
      const lines = [\`Username: @${username || 'learner'}\`, \`Date: ${new Date().toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })}\`, \`Books: ${setup?.books.map(b => b.title).join(', ') || 'Quiz'}\`, \`Difficulty: ${setup?.difficulty || 'Mixed'}\`, \`Time allocated: ${setup?.duration ? setup.duration + ' minutes' : 'No time limit'}\`, \`Time elapsed: ${elapsedText(elapsed)}\`];
      lines.forEach((t, i) => g.fillText(t.slice(0, 78), 80, 490 + i * 48));
      ...
      const text = String(feedback || 'Review the correction section to identify your strongest and weakest areas.').replace(/\\s+/g, ' ').trim(); let line = '', y = 390;
