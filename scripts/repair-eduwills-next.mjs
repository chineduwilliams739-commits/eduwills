...
const imagePatch = `  function makeResultImage(): Promise<Blob> {
    return new Promise((resolve) => {
      ...
      g.font = '900 116px sans-serif'; g.fillText(`__PCT__`, 75, 350); g.font = '700 34px sans-serif'; g.fillText(`__SCORE__`, 80, 405);
      g.font = '600 25px sans-serif';
      const lines = [`__USERNAME__`, `__DATE__`, `__BOOKS__`, `__DIFFICULTY__`, `__TIME__`, `__ELAPSED__`];
...
const imageNew = imagePatch
  .replace('__PCT__', '${pct}%')
  .replace('__SCORE__', '${score}/${qs.length} correct')
  .replace('__USERNAME__', 'Username: @${username || \'learner\'}')
  .replace('__DATE__', 'Date: ${new Date().toLocaleString(undefined, { dateStyle: \'medium\', timeStyle: \'short\' })}')
  .replace('__BOOKS__', 'Books: ${setup?.books.map(b => b.title).join(\', \') || \'Quiz\'}')
  .replace('__DIFFICULTY__', 'Difficulty: ${setup?.difficulty || \'Mixed\'}')
  .replace('__TIME__', 'Time allocated: ${setup?.duration ? setup.duration + \' minutes\' : \'No time limit\'}')
  .replace('__ELAPSED__', 'Time elapsed: ${elapsedText(elapsed)}');
quiz = quiz.replace(/  function makeResultImage\(\): Promise<Blob> \{[\s\S]*?\n  \}\n  async function downloadResult/, imageNew);
