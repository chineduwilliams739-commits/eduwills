...
// Restore the polished Quiz Studio controls that were present before the latest stable rewrite.
...
// Make both activation actions highly visible on the dark/cyan EDUWILLS activation page.
let activation = read('app/dashboard/activation/page.tsx');
activation = activation.replace(/className="inline-flex items-center gap-2 rounded-xl border px-4 py-2\.5 text-sm font-black shadow-lg transition hover:-translate-y-0\.5" style=\{\{background:'#0b1830',borderColor:'#22d3ee',color:'#ffffff',fontWeight:900,letterSpacing:'0\.01em',boxShadow:'0 8px 24px rgba\(2,6,23,\.35\), 0 0 0 1px rgba\(34,211,238,\.08)'\}\}/, "className=\"inline-flex items-center gap-2 rounded-xl border border-cyan-300/50 bg-gradient-to-r from-cyan-400 to-sky-500 px-4 py-2.5 text-sm font-black text-slate-950 shadow-lg transition hover:-translate-y-0.5 hover:brightness-105\"");
...
console.log('EDUWILLS stable implementation applied: broad book knowledge, strict per-book cache-first quiz generation, matching fresh result image, restored Quiz Studio menus/processing UI, visible activation actions, and daily education feed.');
