...
const modernActivationReady = source.includes('redeemThroughBackend') && source.includes('paymentSuccess') && source.includes('Congratulations!') && source.includes('Copy WilliToken');
if (modernActivationReady) {
  console.log('Modern activation page already contains secure redemption, confirmation and copy UI; skipping legacy final rewrite.');
  process.exit(0);
}
...
if (!source.includes('aria-label=\"Activation complete\"')) {
  const modal = `\n      {activationSuccess && (\n        <div className=\"fixed inset-0 z-[240] grid place-items-center bg-slate-950/85 p-4 backdrop-blur-md\">\n          <div role=\"dialog\" aria-modal=\"true\" aria-label=\"Activation complete\" className=\"relative w-full max-w-xl overflow-hidden rounded-[2rem] border border-cyan-300/30 bg-slate-900 p-7 text-white shadow-2xl sm:p-9\">\n            <button type=\"button\" aria-label=\"Close\" onClick={() => setActivationSuccess(null)} className=\"absolute right-4 top-4 rounded-full p-2 text-slate-400 hover:bg-white/10\"><X size={20} /></button>\n            <div className=\"mx-auto grid h-20 w-20 place-items-center rounded-full bg-emerald-400/15 text-emerald-300\"><CheckCircle2 size={46} /></div>\n            <p className=\"mt-5 text-center text-xs font-black uppercase tracking-[.25em] text-emerald-300\">Account activated</p>\n            <h2 className=\"mt-2 text-center text-3xl font-black sm:text-4xl\">🎉 Congratulations!</h2>\n            <p className=\"mx-auto mt-3 max-w-md text-center text-sm leading-6 text-slate-300\">Your WilliToken has been successfully redeemed. Your purchased learning categories are now unlocked.</p>\n            <div className=\"mt-6 rounded-2xl border border-cyan-400/20 bg-cyan-400/5 p-5\"><p className=\"text-center text-[10px] font-black uppercase tracking-[.22em] text-cyan-300\">WilliToken redeemed</p><p className=\"mt-2 break-all text-center font-mono text-2xl font-black tracking-[.22em]\">{activationSuccess.token}</p></div>\n            <div className=\"mt-5 rounded-2xl bg-slate-950/80 p-5\"><p className=\"text-xs font-black uppercase tracking-wider text-slate-500\">Unlocked categories</p><div className=\"mt-3 flex flex-wrap gap-2\">{activationSuccess.categories.length ? activationSuccess.categories.map((category) => <span key={category} className=\"rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-2 text-xs font-black text-emerald-200\">✓ {category}</span>) : <span className=\"text-sm text-slate-400\">Activated learning access</span>}</div><p className=\"mt-4 text-xs text-slate-400\">Access expires: <strong className=\"text-white\">{activationSuccess.expiresAt.toLocaleDateString(undefined, { day: 'numeric', month: 'long', year: 'numeric' })}</strong></p></div>\n            <p className=\"mt-5 text-center text-xs leading-5 text-slate-400\">Your token has been redeemed and cannot be used again. You can now start learning.</p>\n            <div className=\"mt-6 grid gap-2 sm:grid-cols-2\"><button type=\"button\" onClick={() => setActivationSuccess(null)} className=\"rounded-2xl bg-emerald-400 px-5 py-3.5 text-sm font-black text-slate-950\">Continue learning</button><a href={\`${BASE}/dashboard/\`} className=\"inline-flex items-center justify-center rounded-2xl border border-white/10 px-5 py-3.5 text-sm font-black text-white\">Go to dashboard</a></div>\n          </div>\n        </div>\n      )}\n`;
  const marker = '    </main>';
