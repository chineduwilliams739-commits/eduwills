...
function researchFallback(books:QuizBook[],research:string,count:number,previous:string[]):QuizQuestion[]{const facts=[...books.flatMap(b=>curated[normalize(b.title)]||[]),...research.split(/\n+/).filter(x=>x.length>35)].map(x=>x.replace(/^(Google Books|Open Library|Wikipedia|Wikidata|Categories|Subjects|Publication):\s*/,'').trim()).filter(Boolean);const out:QuizQuestion[]=[];const recent=new Set(previous.map(fingerprint));for(let i=0;i<facts.length&&out.length<count;i++){const fact=facts[i];const stem=`Which statement is supported by the available information about ${books[i%books.length].title}?`;const options=[fact,...facts.filter(x=>x!==fact).slice(0,3)];while(options.length<4)options.push('This information is not supported by the available research.');const q={question:stem+' Consider the research evidence.',options:options.slice(0,4),answer:0,explanation:'Generated from verified research because the AI service did not return a complete batch.'};if(!recent.has(fingerprint(q.question))&&!out.some(x=>similar(x.question,q.question)))out.push(q)}return out}
